...
export const paymentService = {
  createPaymentPreference: async (activityId: string): Promise<string> => {
    ...
    try {
      ...
      const initPoint = paymentResponse.initPoint || 
                       paymentResponse.init_point ||
                       paymentResponse.data?.initPoint ||
                       paymentResponse.data?.init_point;
      
      if (!initPoint) {
        console.error('❌ Respuesta de pago:', paymentResponse);
        throw new Error('No se pudo obtener el link de pago. Respuesta inesperada del servidor.');
      }
  ...
  getPaymentHistory: async (): Promise<Payment[]> => {
    try {
      ...
      if (!Array.isArray(payments)) {
        console.warn('⚠️ Respuesta no es un array:', rawData);
        return [];
      }
      ...
      if (error.response?.status === 401) {
        console.warn('Usuario no autenticado, retornando array vacío');
        return [];
      }
